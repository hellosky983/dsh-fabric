...
export function apply(ctx) {
  ...
  const schema = () => ({
    ...
    kinds: [
      { kind: 'tool', needsCode: true, shape: { name: 'string', description: 'string', parameters: 'ParameterSchemaSpec DSL', execute: 'async (args, exec) => json' } },
      { kind: 'skill', needsCode: false, shape: { name: 'kebab-case', description: 'string', whenToUse: 'string', content: 'markdown' } },
      { kind: 'command', needsCode: false, shape: { command: 'CommandDefinition' } },
      ...
      { kind: 'llmAdapter', needsCode: true, shape: { providers: 'string[]', adapter: 'LlmAdapter' } },
      { kind: 'subagentProvider', needsCode: true, shape: { provider: 'SubagentProvider' } },
      { kind: 'webRoute', needsCode: true, shape: { route: 'WebRoute' } },
      { kind: 'settingsNamespace', needsCode: false, shape: { ns: 'SettingsNamespace', schema: 'z schema', options: 'SettingsRegisterOptions' } },
      { kind: 'projection', needsCode: false, shape: { definition: 'ProjectionDefinition' } },
    ],
    seams: SEAMS,
  ...
  if (skills) {
    skills.register({
      ...
      content: [
        ...
        '- command: { kind:"command", command: CommandDefinition }',
        '- eventListener: { kind:"eventListener", event, listener(...) }',
        '- llmAdapter: { kind:"llmAdapter", providers, adapter }',
        '- subagentProvider: { kind:"subagentProvider", provider }',
        '- webRoute: { kind:"webRoute", route }',
        '- settingsNamespace: { kind:"settingsNamespace", ns, schema, options }',
        '- projection: { kind:"projection", definition }',
        '',
